SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE pr_getussearch_en (PV_REFCURSOR IN OUT PKG_REPORT.REF_CURSOR,
TellerId varchar2, BranchId varchar2, SearchFilter varchar2)
is
    v_sql clob;
    /*
    18/08/2017 TruongLD Add, xu ly song ngu cho cac GD tren man hinh chinh
    */
begin
    ...
    if TellerId='0001' THEN --Admin he thong
        ...
        SELECT lg0.CFFULLNAME CFFULLNAME,lg0.CFCUSTODYCD IDAFACCTNO, NVL(SB.SYMBOL,''---'') CODEID, tltx.en_txdesc NAMENV,
          lg0.CAREBYGRP,lg0.AUTOID,A1.EN_CDCONTENT DELTD,lg0.TXNUM,lg0.TXDATE,
          lg0.BUSDATE,lg0.BRID,lg0.TLTXCD,A0.EN_CDCONTENT TXSTATUS,
        ...
        FROM (select * from tllog where batchname = ''DAY'' or tltxcd in (''0381'',''3375'')) lg0, ALLCODE A0, ALLCODE A1,tltx,SBSECURITIES SB,
          (
        ...
        SELECT distinct * FROM (SELECT lg0.CFFULLNAME CFFULLNAME, lg0.CFCUSTODYCD IDAFACCTNO, NVL(SB.SYMBOL,''---'') CODEID,
            tltx.txdesc NAMENV,lg0.CAREBYGRP,lg0.AUTOID,A1.EN_CDCONTENT DELTD,lg0.TXNUM,
            lg0.TXDATE,lg0.BUSDATE,lg0.BRID,lg0.TLTXCD,A0.EN_CDCONTENT TXSTATUS,
             ((CASE WHEN lg0.TXSTATUS=''7'' THEN ''4'' ELSE  lg0.TXSTATUS END)  ) TXSTATUSCD,
            lg0.TXDESC,lg0.MSGACCT ACCTNO, lg0.MSGAMT AMT, lg0.TLID,lg0.CHID,lg0.CHKID,lg0.OFFID,tltx.DELALLOW,
            ...
            FROM (select * from tllog where batchname = ''DAY'' or tltxcd in (''0381'',''3375'')) lg0,
            ...
            WHERE NOT (lg0.TLTXCD LIKE ''%71''OR lg0.TLTXCD LIKE ''%72'' OR lg0.TLTXCD LIKE ''%6631'' OR lg0.TLTXCD IN (''0011'',''1175'',''8873'',''2282'',''1177''))
            AND (lg0.tltxcd in(''0381'',''1101'',''1141'',''1153'') or  lg0.CAREBYGRP IS NULL OR (lg0.CAREBYGRP LIKE ''%'' || TLCAREBY.GRPID || ''%''))
            ...
            AND (
                ...
                OR (
                    lg0.TLTXCD IN (
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''T'' AND TLLIMIT >=0 and AUTHID= ''' || TellerId || ''' AND AUTHTYPE=''U''
                        UNION ALL
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''T'' AND TLLIMIT >=0 and AUTHID IN  (select grpid from grpuser)  AND AUTHTYPE=''G'')
                    )
                OR (
                    lg0.TLTXCD IN (
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''A'' AND TLLIMIT >=0 and AUTHID= ''' || TellerId || ''' AND AUTHTYPE=''U''
                        UNION ALL
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''A'' AND TLLIMIT >=0 and AUTHID IN  (select grpid from grpuser)  AND AUTHTYPE=''G'')
                    )
                OR (
                    lg0.TLTXCD IN (
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''C'' AND TLLIMIT >=0 and AUTHID= ''' || TellerId || ''' AND AUTHTYPE=''U''
                        UNION ALL
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''C'' AND TLLIMIT >=0 and AUTHID IN  (select grpid from grpuser)  AND AUTHTYPE=''G'')
                    )
                OR (
                    lg0.TLTXCD IN (
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''R'' AND TLLIMIT >=0 and AUTHID= ''' || TellerId || ''' AND AUTHTYPE=''U''
                        UNION ALL
                        SELECT DISTINCT TLTXCD FROM TLAUTH WHERE TLTYPE=''R'' AND TLLIMIT >=0 and AUTHID IN (select grpid from grpuser) AND AUTHTYPE=''G'')
                    )
            )
            )TLLOG WHERE 0=0 ';
    end if;
