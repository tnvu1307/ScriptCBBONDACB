SET DEFINE OFF;
CREATE OR REPLACE FUNCTION fn_getparentgroupmenu_vn(pv_cmdid IN varchar2, pv_cmdtype in varchar2, pv_module in varchar2, pv_last in varchar2)
    RETURN varchar2 IS
    l_return varchar2(1000);
    l_cmdname varchar2(100);
    l_prid  varchar2(100);
BEGIN
    dbms_output.put_line('pv_cmdid:');
    ...
    select cmdname into l_cmdname from cmdmenu where cmdid = l_prid;

    if  l_prid <> 'XXXXXXXXXX' then
        l_return:= fn_getparentgroupmenu_vn(l_prid, 'M', null, 'N') || '/' || l_cmdname;
    else
        l_return:= l_cmdname;
        return l_return;
    end if;
