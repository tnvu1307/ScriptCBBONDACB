SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE pr_genviewtest_locpt (p_tablename varchar2)
         IS
   ...
   BEGIN
      ...
      VALUES(p_tablename,'Inquiry for table '||p_tablename,'Inquiry for table '||p_tablename,'select * from Fiin.'||p_tablename||' where 0=0 ','p_tablename',NULL,NULL,NULL,NULL,50,'N',30,NULL,'Y','T',NULL,'N',NULL);
    ...
    loop
      ...
      VALUES(v_row.column_id,v_row.column_name,v_row.column_name,case when instr(v_row.data_type,'TIMESTAMP')>0 then 'D'
                               ...
                               else 'C' end,p_tablename,v_row.data_length,NULL,
