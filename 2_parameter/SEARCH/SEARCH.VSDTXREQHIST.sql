SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('VSDTXREQHIST','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT, CHKSCOPECMDSQL) Values   ('VSDTXREQHIST', 'Tra cứu lịch sử thông tin chi tiết điện gửi VSD', 'Manage history of request messages sent to VSD', 'SELECT REQ.REQID,REQ.OBJNAME,SUBSTR(REQ.TRFCODE,1,3)TRFCODE,REQ.OBJKEY,REQ.TXDATE,REQ.MSGACCT,
...
LEFT JOIN (
    SELECT V.REQID, AC.CDCONTENT, AC.EN_CDCONTENT FROM VSDTXREQDTLHIST V 
...
LEFT JOIN (
    SELECT V.REQID, AC1.CDCONTENT, AC1.EN_CDCONTENT FROM VSDTXREQDTLHIST V 
...
LEFT JOIN (
    SELECT * FROM VSDTXREQDTLHIST WHERE FLDNAME in (''REPORTID'',''RPTID'')
...
) TLG ON TLG.TXNUM = REQ.OBJKEY AND TLG.TXDATE = REQ.TXDATE', 'VSDTXREQHIST', 'frmVSDTXREQHIST', 'TXDATE DESC,REQID DESC', '', 0, 5000, 'Y', 30, '', 'Y', 'T', '', 'N', '', '');COMMIT;